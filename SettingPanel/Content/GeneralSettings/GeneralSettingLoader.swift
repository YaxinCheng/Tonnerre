...
struct GeneralSettingLoader {
  let settings: [(SettingKey, String, String, NSUserInterfaceItemIdentifier)] = [
    python, warnBeforeExit, cbLimit
  ]
  
  private static let python = (SettingKey.python,
                               "Python Interpreter",
  ...
  private static let warnBeforeExit = (SettingKey.warnBeforeExit,
                                       "Warn before exit",
                                       "When enabled, user double click \(String.CMD) Q to exit the program",
                                       NSUserInterfaceItemIdentifier.boolCell)
  private static let cbLimit = (SettingKey.clipboardLimit,
                                "ClipboardService",
