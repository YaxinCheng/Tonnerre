...
enum Logger {
  ...
  static func info(file: String, _ message: StaticString, _ args: CVarArg...) {
    let logger = OSLog(subsystem: bundleId, category: file)
    os_log(.info, log: logger, message, args)
  }
  
  static func debug(file: String, _ message: StaticString, _ args: CVarArg ...) {
    let logger = OSLog(subsystem: bundleId, category: file)
    os_log(.debug, log: logger, message, args)
  }
  
  static func error(file: String, _ message: StaticString, _ args: CVarArg ...) {
    let logger = OSLog(subsystem: bundleId, category: file)
